...
//				else{
	...
	function document_onload(){
		...
		$("popup").faceTip="惊讶,撇嘴,色,流泪,大哭,尴尬,发怒,调皮,谄笑,微笑,难过,酷,生病,吐,偷笑,吻,舔,可爱,呲牙,淘气,流口水,窘迫,晕,闭嘴,饿,困,惊恐,汗,憨笑,大兵,思考,疑问,好,不,强,弱,赞,鄙视,握手,再见,叹气,敲打,吃饭,电话,献花,加油,猪头,猫咪,蛋糕,咖啡,玫瑰,凋谢,爱心,心碎,礼物,下雨,炸弹,足球,便便,雪人";
	...
	function initTool(str){
		var fontname="宋体,黑体,楷体,隶书,幼圆,Arial,Arial Black,Courier,Tahoma,Verdana",a;
		var fontsize="默认,2;小,1;正常,2;大,4;更大,5;特大,6;极大,7";
		...
		if(str.inStr("bold"))
			createBtn("iconEditor_bold.gif","粗体",function(){exec("bold")});
		if(str.inStr("italic"))
			createBtn("iconEditor_italic.gif","斜体",function(){exec("italic")});
		if(str.inStr("left"))
			createBtn("iconEditor_left.gif","居左对齐",function(){exec("justifyleft")});
		if(str.inStr("center"))
			createBtn("iconEditor_center.gif","居中对齐",function(){exec("justifycenter")});
		if(str.inStr("right"))
			createBtn("iconEditor_right.gif","居右对齐",function(){exec("justifyright")});
		if(str.inStr("removeformat"))
			createBtn("iconEditor_erase.gif","橡皮擦",function(){exec("removeformat")});
		if(str.inStr("link"))
			createBtn("iconEditor_link.gif","插入超链接",function(){special("createlink")});
		if(str.inStr("color"))
			createBtn("iconEditor_color.gif","字体颜色",function(){$("popup").show("color",this)},"color");
		if(str.inStr("bgcolor"))
			createBtn("iconEditor_bgcolor.gif","背景颜色",function(){$("popup").show("bgcolor",this)},"bgcolor");
		if(str.inStr("image"))
			createBtn("iconEditor_image.gif","插入图片",function(){special("insertimage")});
		if(str.inStr("face"))
			createBtn("iconEditor_smile.gif","插入表情",function(){$("popup").show("face",this)},"face");
	}
	...
	function special(cmd){
		if("createlink,insertimage".inStr(cmd)){
			if(cmd=="createlink"){
				...
				info="插入图片链接\n必须为http://开头，支持格式jpg,gif,png,bmp";
				s1="http\:\/\/";
				s2="\.(jpg|gif|png|bmp)";
			}
			if(browser.msie)
				str=window.showModalDialog("input.htm",{"title":(cmd=="createlink"?"请输入超链接路径<br />必须为http://和ftp://开头":"请输入图片路径<br />必须为http://开头，支持格式jpg,gif,png,bmp"),"value":"http://"},"dialogwidth:354px;dialogheight:152px;status:no;help:no")
			else
