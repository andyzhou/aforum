...
#ifdef ZTS
/* When comiling a PHP extension */
#include "php.h"
#define m_malloc(n) emalloc(n)
#define m_free(p) efree(p)
#define m_realloc(p, n) erealloc(p, n)
#else
...
/* Load template file */
int tpl_load(request_rec *r, tpl_t *tpl, const char *filename)
{
    char *buffer;
    int len;
	...
	apr_status_t rv;
	apr_finfo_t finfo;
	apr_file_t *tplft = NULL;
	apr_off_t offset = 0;
	apr_size_t taille = 0;
	
	
	//use apache2 api to open file
	r->filename = ap_psprintf(r->pool, filename);
	rv = apr_file_open(&tplft, r->filename, APR_READ, APR_UREAD | APR_GREAD, r->pool);
	...
	//get file size
	rv = apr_file_info_get(&finfo, APR_FINFO_SIZE, tplft);
	len = finfo.size;
	
	//ap_rprintf(r, "<h1>len:%d</h1>", len);
	...
	*/
	...
	buffer = (char *)apr_palloc(r->pool, sizeof(char) * (len + 1));
	
	//read tpl content
	apr_file_read(tplft, buffer, &len);
	apr_file_close(tplft);
	
	//ap_rprintf(r, "buf:%s", buffer);
