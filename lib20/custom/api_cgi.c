...
int getSpecGetData(request_rec* r, apr_table_t** _param_list)
{
	...
	*_param_list = apr_table_make(r->pool, 1);
	...
	while (*_spec_get_data != '\0')
	{
		...
		//ap_rprintf(r, "<h1>key:%s, val:%s</h1>", key_str, key);
					  ...
					  int check_data_val)
{
	...
	//分析数据相关参数
	*data_param_list = ap_make_table(r->pool, 1);

	if(!_param_str || !(*_param_str) || !strcmp(_param_str, "(null)"))
	{
		return 0;
	}
	...
	if(check_data_val && (!data_val || !(*data_val)))
	{
		return 0;
	}
	...
	while (*_param_str != '\0')
	{
		...
		if ((val = strchr(key, '=')) != NULL)
		{
			*val++ = '\0';
			...
			//ap_rprintf(r, "<h1>key_1:%s, val_1:%s</h1>", key, val);
		}
	}
...
//接口类型：private
int parse_multi_data(request_rec* r, int nBytes, char* m_lpBuffer, apr_table_t** _param_list)
{
	/*******************************************************
